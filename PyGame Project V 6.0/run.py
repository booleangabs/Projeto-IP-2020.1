...
class Game:
    ...
    def load_data(self):
        ...
        self.papiro_img = pg.image.load(os.path.join(self.img_folder, options['PAPIRO'])).convert_alpha()
    ...
    def draw(self):
        ...
        # calc
        pg.draw.rect(self.screen, barColor, (20, 20, 250, 20))
        pg.draw.rect(self.screen, (234, 12, 51), (20, 25, (calcScore*25), 10))
        # md
        pg.draw.rect(self.screen, barColor, (20, 45, 250, 20))
        pg.draw.rect(self.screen, (46, 253, 57), (20, 50, (mdScore*25), 10))
        # avlc
        pg.draw.rect(self.screen, barColor, (20, 70, 250, 20))
        pg.draw.rect(self.screen, (0, 255, 255), (20, 75, (avlcScore*25), 10))
        ...
        text = self.font.render(f' {calcCount} ', 1, textColor, bgColor)
        self.screen.blit(text, (int((self.screen.get_width() - text.get_width()) // 2) - 230, 22))
        text = self.font.render(f' {avlcCount} ', 1, textColor, bgColor)
        self.screen.blit(text, (int((self.screen.get_width() - text.get_width()) // 2) - 230, 72))
        text = self.font.render(f' {mdCount} ', 1, textColor, bgColor)
        self.screen.blit(text, (int((self.screen.get_width() - text.get_width()) // 2) - 230, 47))

        text = self.font.render(f' Nota: {partialScore:.2f}({self.player.scrollCount}) ', 1, textColor, bgColor)
        self.screen.blit(text, (int((self.screen.get_width() - text.get_width()) // 2) + 400, 20))
        text = self.font.render(f' Última Nota: {lastScore} ', 1, textColor, bgColor)
        self.screen.blit(text, (int((self.screen.get_width() - text.get_width())//2) + 400, 45))
        text = self.font.render(f' Faltam {(self.maxTime - self.elapsed) / 6:.1f} horas ', 1, textColor, bgColor)
        self.screen.blit(text, (int((self.screen.get_width() - text.get_width()) // 2) + 400, 70))
        pg.display.flip()
    ...
    def show_end_screen(self, win):
        ...
        cover = pg.Surface((window['WIDTH'], window['HEIGHT'])).convert_alpha()
        cover.fill((15, 15, 15, 240))
        ...
        while True:
            ...
            strings = [f" Nota...........{partialScore:.2f}({self.player.scrollCount})",
                       ...
                       f"",
                       f" Pressione 'ESC' para finalizar o jogo"]
            height = 0
