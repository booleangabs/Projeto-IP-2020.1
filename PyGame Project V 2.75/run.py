            ...
            for event in pg.event.get():
                ...
                if event.type == pg.KEYDOWN:
                    if event.key == pg.K_KP_ENTER or event.key == pg.K_RETURN:
                        pg.mixer.stop()
                        return
            font = pg.font.Font("C:\Windows\Fonts\Courbd.ttf", 40)
            text = font.render(f'O monitorado', 1, (225, 50, 50))
            self.screen.blit(text, (int((self.screen.get_width() - text.get_width())//2), int((self.screen.get_height()-150)//2)))
            text = font.render(f'Aperte \'Enter\' para começar', 1, (175, 175, 215))
            self.screen.blit(text, (int((self.screen.get_width() - text.get_width())//2), int(self.screen.get_height()//2)))
            pg.display.flip()
